# let's create bpd and pediatric cardiac operations

# names and dirs #####

setwd("/home/pvartiai/RSV/")
...
hilmo_kids_tmp <- fread(paste0(hilmo_dir, tmp_name)) %>% as_tibble %>%
  filter(TNRO %in% all_bds$TNRO)
...
write_feather(heartop_hilmo_entries, "/home/pvartiai/RSV/data/predictors/heart_operations.feather")
...
# NOT RUN
# FOR THE SEVERE ONES, WE ARE TRYING NEW CHD CLASSIFICATION BASED ON ONLY OPERATIONS
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
only_common_chds <- common_chd_and_op %>%
  mutate(only_vsd = ifelse(
                            vsd == 1 &
                            common_asd == 0 &
                            other_chd == 0 &
                            chd_oper_1mo == 0 & 
                            chd_oper_1year == 0, 1, 0)) %>%
  mutate(only_common_asd = ifelse(
                                  common_asd == 1 & 
                                  other_chd == 0 &
                                  vsd == 0 & 
                                  chd_oper_1mo == 0 & 
...
downs <- bind_rows(early_hilmo_chromo, chromo_mbr) %>%
  distinct() %>%
  mutate(down = 1)
