...
# select those born after 1.6.1997 according to dates
mbr_raw <- mbr_raw %>%
    filter(LAPSEN_SYNTYMAPVM >= as.Date("1997-06-01")) %>%
    filter(LAPSEN_SYNTYMAPVM <= as.Date("2020-05-31"))
...
#  n of dead = ..., all deceased are in the medical birth registry 
