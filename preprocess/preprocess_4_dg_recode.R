...
mbr_c_long_q <- mbr_c_long_q %>%
    mutate(temp_q_diag = case_when(
        ...
        # Q75-79
        q_diag %in% c("Q75", "Q76", "Q77", "Q78") ~ "Q75-78",
...
write.csv(dg_counts, "neonat_and_preg_dg_counts.csv")
...
all_ids_and_composites <- mbr_c %>% select(TNRO, smoking, gest_days)
